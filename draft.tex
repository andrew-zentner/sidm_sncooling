...
brehmsstrahlung \cbm{This is probably stupid, but shouldn't it be bremsstrahlung, without the first h?}, and radiated. This non-standard energy loss mechanism can result in an energy loss rate 
...
signs of the diagrams. I tried to fix them. Please go over this VERY carefully.}
\cbm{Fixed a couple of subscripts. I think it should be okay now.}
...
\begin{eqnarray}
	...
	M_3 &=& \frac{4 M_N}{ m_\pi} \frac{f_{\mathrm{pp}}^2\, e\, \varepsilon}{k^2-m_\pi^2}  \frac{1}{m_{A^\prime}^2 - 2 \qa \cdot p_2}\ 
	\bar{u}(p_3) \gamma_5 u(p_1)\ \bar{u}(p_4) \gamma_5 (\slashed{p}_2 - \qa +M_N)\slashed \epsilon  u(p_2), \\
	M_4 &=& -\frac{4 M_N}{ m_\pi} \frac{f_{\mathrm{pp}}^2\, e\, \varepsilon}{l^2-m_\pi^2}  \frac{1}{m_{A^\prime}^2 - 2 \qa \cdot p_2}\ 
	...
	\bar{u}(p_3) \gamma_5 u(p_4)\ \bar{u}(p_2) \slashed{\epsilon} (\slashed{p}_4 + \qa +M_N)\gamma_5 u(p_1),\\
...
for the spin-averaged squared matrix element. Our result contains over 200 terms, 
...
be representative of supernova cores. \cite{snova_core} \arz{Need to put a citation here for the properties of supernova cores. 
I can probably find something in the literature, but please put something here if you have a citation.} \cbm{That okay?}


We performed the phase space integrals using the Monte Carlo routines in the {\tt CUBA} library. \cite{hahn_suave}
...
%
\begin{equation}
\left( \frac{L_{\mathrm{max}}}{M} \right) \approx 10^{19} \, \frac{\mathrm{erg}}{\mathrm{g}\, \mathrm{s}}
\end{equation}
%
...
of the core, so decays contribute little to the cooling of the stellar core. 
...
l = \beta \tau_{\mathrm{A}'} = \frac{3 q_{\mathrm{A}'}}{m_{\mathrm{A'}}^2 \varepsilon^2},
\eeq
...
\exp \left( - \frac{r_{\mathrm{decay}} m_{\mathrm{A'}}^2 \varepsilon^2}{3 q_{\mathrm{A'}}} \right),
\eeq
%
...
because the density of the supernova drops drastically around that point. \cite{snova_density} 
\arz{We need a citation for this statement, but I do not know one off of the top of my head.}\cbm{And that?}
...
which are the opacities due to inverse brehmsstrahlung for proton-proton and proton-neutron 
...
\tau &=& \int_{r_x}^{\infty}\, \tau_\mathrm{R} \left( \frac{R}{r} \right)^{3n/2} \dd r \\
 ...
 $10$~km. Have you done this?} %On second thought, put that separately%
...
in the $\varepsilon$-$m_{\mathrm{A'}}$ plane shaded in blue. 
\arz{Cameron, can you describe clearly each line in this caption. Yellow is 
too hard to see, so please choose another color for this line.} \cbm{Isn't that what this whole section of text does? It seems redundant}
}
...
of the brehmsstrahlung rate in Ref.~\cite{dent_etal12}. 
...
nuclear scattering data in the estimation of the brehmsstrahlung rate. 
...
Fig.~\ref{fig:constraints} shows the lower bound on the excluded region that 
...
\arz{CBM was supported somehow by AKL.}
