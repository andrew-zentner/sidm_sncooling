...
be specified explicitly in the text. Please do so as soon as possible.
...
This suggests that we should use the non-relativistic limit here. Do I misunderstand something? Let us address this 
...
\bea 
\tau &=& \int_{r_x}^{\infty}\, \tau_R \left( \frac{R}{r} \right)^{3n/2} \dd r \\
 &=& \frac{\tau_r}{\frac{3n}{2}-1} \left( \frac{T_A}{T_R} \right)^{(9/2-3/n)}. 
 \eea
 The bound on the coupling is obtained by requiring  $ \tau(\varepsilon,m_A) \le 2/3 $
...
\begin{figure*}[th]
\includegraphics[width=14cm]{constraint.pdf}
\caption{
...
too hard to see, so please choose another color for this line.}
}
