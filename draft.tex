...
be incorrect. 
%We have not been able to identify the errors which led to these incorrect expressions; however,  there must have been several oversights in the analysis of Ref.~\cite{dent_etal12}. 
...
%Unfortunately, with the kinematical relations above, the squared amplitude does not yield a tidy expression for the spin-averaged squared matrix element. 
...
\tau &=& \int_{r_x}^{\infty}\, \tau_\mathrm{R} \left( \frac{R}{r} \right)^{3n/2} \dd r \\
 ...
 \eea
 The bound on the coupling is obtained by requiring  $ \tau(\varepsilon,m_\mathrm{A'}) \le 2/3 $. Note that strictly speaking we should have determined $ r_{\mathrm{dp}} $ rather than assuming a value. To check if our assumption is valid we can go backwards through the calculation and determine the value of $ r_{\mathrm{dp}} $ at which $ \tau = 2/3$. Doing so we find that our choice of 10 km is roughly correct.
