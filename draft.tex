...
exclusion band shown in Figure~\ref{fig:bound1}, which we will discuss in more detail 
below. If all of the $A'$ produced in the core left the supernova freely, the bound of values of 
$\varepsilon$ higher than those given by Eq.~(\ref{eq:bound1}) would be ruled out. However, 
as we have already mentioned, $\varepsilon$ can become sufficiently large that only a negligible 
amount of energy actually leaves the supernova core because of either $A'$ decays or scattering. 
These considerations place an upper limit on the values of $\varepsilon$ for which this constraint 
applies, and we discuss these effects in the next two subsections.
...
(or lower bound on the excluded coupling) from the production on $A'$ bosons. 
In order to function as an effective cooling channel, enough of the dark gauge bosons 
must escape the supernova. As the coupling increases, so do processes that prevent this escape, 
...
\arz{Cameron, try to rephrase this. This is not the lifetime, but rather the 
mean distance to a decay.}
%
\beq
...
The luminosity in $A'$ will be an increasing function of $\varepsilon$ until decays suppress the gauge boson luminosity, 
...
where the $A'$ mean free path becomes larger than the typical size of the supernova core. 
In this case the luminosity is given simply by Stefan's law, 
\beq
\mathcal{L}_t  = 4\pi r^2 T_A^4 \sigma,
\eeq
where $r$ is now the radius of the emitting shell and $T_A$ its temperature. 
We estimate the radius of this dark photosphere as $r=10$ km, 
because the density of the supernova drops drastically around that point. 
After taking that value for r, the bound on the luminosity translates into a bound on $ T_A $ 
\beq
T_A \le 9.586\rm\ MeV.
\eeq
